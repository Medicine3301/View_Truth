...
CREATE TABLE `comments` (
  ...
  `content` text,
  PRIMARY KEY (`comm_id`),
  KEY `fk_comments_pid` (`pid`),
  KEY `fk_comments_nid` (`nid`),
  CONSTRAINT `fk_comments_nid` FOREIGN KEY (`nid`) REFERENCES `news` (`news_id`),
  CONSTRAINT `fk_comments_pid` FOREIGN KEY (`pid`) REFERENCES `post` (`pid`)
...
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `post_comment_count` AFTER INSERT ON `comments` FOR EACH ROW BEGIN
  if NEW.pid is not null then   #check pid is not null
     UPDATE post
...
-- Dump completed on 2024-10-22  0:47:16
