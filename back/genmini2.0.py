...
class GeminiVerificationSystem:
    ...
    def generate_factual_prompt(self, content: str) -> str:
        """生成事實檢查提示詞"""
        return f"""
        以專業事實檢查員的身份，評估以下信息的可信度。請關注事實聲明、數據引用和可驗證的內容。
    ...
    def generate_critical_prompt(self, content: str) -> str:
        """生成批判性分析提示詞"""
        return f"""
        請採用批判性思維專家的角度，分析以下信息的可靠性。特別關注可能的錯誤、誤導或邏輯謬誤。
    ...
    def generate_balanced_prompt(self, content: str) -> str:
        """生成平衡視角提示詞"""
        return f"""
        請以中立的觀察者身份，平衡地評估以下信息的可信度。考慮支持和反對的觀點。
    ...
    def generate_source_checking_prompt(self, content: str) -> str:
        """生成來源檢查提示詞"""
        return f"""
        請扮演信息來源專家，評估以下信息的來源可靠性。關注信息來源、引用和權威性。
...
def main():
    ...
    # 測試信息
    test_content = """獨家／熱炒店遇「免費白飯大隊」！30學生「狂掃2桶飯」嗆沒吃飽還洗1星
